<template>
  <div class="row post-title px-5">
    <div class="col-12 col-md-6 print-d-none">
      <LayoutMediaFocus :source="getStrapiUrl(post.attributes.Image)" :title="post.attributes.Title" />
    </div>
  ...
  <article class="row p-5 post-display">
    <div class="col-3 d-none d-lg-block">
      <BlogTOC :content="post.attributes.Content" id="toc" :sticky="true" />
    </div>
    <div class="col-12 col-md-8 col-lg-6" data-bs-spy="scroll" data-bs-target="#toc" data-bs-smooth-scroll="true">
      <BlogTOC class="d-none print-d-only" id="print-toc" :content="post.attributes.Content" :sticky="false" />
      <span class="fst-italic fs-5">{{ formatDateFull(post.attributes.publishDate) }}</span>
      <BlogRichText :block="post.attributes.Content" />
      <BlogRichText :block="post.attributes.CTA" />
    </div>
    <div class="col-12 col-md-4 col-lg-3">
      <div class="position-lg-sticky top-0">
        <LayoutPostListSidebar title="Related Articles" :posts="relatedPosts" />
      </div>
    </div>
  </article>
</template>
